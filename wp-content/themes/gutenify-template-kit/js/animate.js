...
jQuery(function ($) {
  ...
  function gutenify_template_kit_slider_animation(selector) {
  ...
  $(document).on('gutenifySlideinit', function () {
    var observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          gutenify_template_kit_slider_animation('.swiper-slide-active');
          return;
        }
      });
    });
    observer.observe(document.querySelector('.swiper-container'));
  }); // gutenify_template_kit_slider_animation('.swiper-slide:first-child');
