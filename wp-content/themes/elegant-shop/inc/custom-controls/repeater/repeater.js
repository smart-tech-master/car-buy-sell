/*jshint -W065 */
var elegantshopproRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['elegant-shop-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
		...
		// The current value set in Control Class (set in Elegant_Shop_Pro_Control_Repeater::to_json() function)
	...
	setImageInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.elegant-shop-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.elegant-shop-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ){
		...
		$targetDiv.find( '.elegant-shop-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ){
		...
		$targetDiv.find( '.elegant-shop-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ){
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Elegant_Shop_Pro_Control_Repeater::render_content() ).
		...
		if ( template ) {
			...
			newRow = new elegantshopproRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'elegant_shop_get_fontawesome_ajax',
					elegant_shop_customize_nonce: elegant_shop_customize.nonce
				},
				beforeSend: function(){
