 ...
 */
wp.customize.controlConstructor['blossom-studio-typography'] = wp.customize.Control.extend( {
    ready: function(){        
        'use strict';
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.blossom-studio-variant-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.blossom-studio-variant-wrapper' ).css( 'display', 'block' );
				}
