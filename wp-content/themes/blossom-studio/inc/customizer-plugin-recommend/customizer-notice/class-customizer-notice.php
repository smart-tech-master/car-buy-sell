<?php
/**
 * Blossom Studio Customizer Notification System.
 *
 ...
 */
class Blossom_Studio_Customizer_Notice {
	...
	/**
	 * The single instance of Blossom_Studio_Customizer_Notice
	 *
	 * @var Blossom_Studio_Customizer_Notice $instance The Blossom_Studio_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Blossom_Studio_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Blossom_Studio_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Blossom_Studio_Customizer_Notice ) ) {
			self::$instance = new Blossom_Studio_Customizer_Notice;
	...
	public function setup_config() {
		...
		$blossom_studio_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$blossom_studio_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'blossom-studio-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'blossom-studio-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), BLOSSOM_STUDIO_THEME_VERSION, true );
		wp_localize_script(
			'blossom-studio-customizer-notice', 
            'customizer_notice_data', 
            array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'blossom-studio' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the Blossom_Studio_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Blossom_Studio_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Blossom_Studio_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'blossom_studio_show_recommended_plugins', $show_recommended_plugins );
		}
