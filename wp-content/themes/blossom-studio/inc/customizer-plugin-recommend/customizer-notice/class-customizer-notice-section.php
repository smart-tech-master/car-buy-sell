<?php
/**
 * Blossom Studio Customizer Notification Section Class.
 *
 * @package Blossom_Studio
 */

/**
 * Blossom_Studio_Customizer_Notice_Section class
 */
class Blossom_Studio_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'blossom_studio_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'blossom_studio_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $blossom_studio_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'blossom_studio_show_recommended_plugins' );

		foreach ( $blossom_studio_recommended_plugins as $slug => $plugin_opt ) {
			...
			$blossom_studio_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$blossom_studio_recommended_plugin['class'] = 'active';
			} else {
				$blossom_studio_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$blossom_studio_recommended_plugin['button_class'] = 'install-now button';
					$blossom_studio_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$blossom_studio_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$blossom_studio_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$blossom_studio_recommended_plugin['button_class'] = 'deactivate-now button';
					$blossom_studio_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$blossom_studio_recommended_plugin['id'] = $slug;
			$blossom_studio_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$blossom_studio_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$blossom_studio_recommended_plugin['description'] = $info->short_description;
			}

			$blossom_studio_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $blossom_studio_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="blossom-studio-recommeded-actions-container blossom-studio-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="blossom-studio-recommeded-actions">
