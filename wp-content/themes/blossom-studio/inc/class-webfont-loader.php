...
if ( ! class_exists( 'Blossom_Studio_WebFont_Loader' ) ) {
	 ...
	 * Download webfonts locally.
	 */
	class Blossom_Studio_WebFont_Loader {
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'blossom_studio_local_font_files', $local_font );
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'blossom_studio_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'blossom_studio_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'blossom_studio_get_local_fonts_subfolder_name', 'fonts' );
			}
...
if ( ! function_exists( 'blossom_studio_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function blossom_studio_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Blossom_Studio_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'blossom_studio_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function blossom_studio_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Blossom_Studio_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
...
if( ! function_exists( 'blossom_studio_load_preload_local_fonts' ) ):
/**
...
* @param string $format
*/
function blossom_studio_preload_local_fonts( $url, $format = 'woff2' ) {
    // Check if cache font files present or not
    $local_font_files = get_site_option( 'blossom_studio_local_font_files', false );

    if ( is_array( $local_font_files ) && ! empty( $local_font_files ) ) {
        $font_format = apply_filters( 'blossom_studio_local_google_fonts_format', $format );
