<?php
/**
 * Seva Lite Customizer Notification Section Class.
 *
 * @package Seva_Lite
 */

/**
 * Seva_Lite_Customizer_Notice_Section class
 */
class Seva_Lite_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'seva_lite_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'seva_lite_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $seva_lite_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'seva_lite_show_recommended_plugins' );

		foreach ( $seva_lite_recommended_plugins as $slug => $plugin_opt ) {
			...
			$seva_lite_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$seva_lite_recommended_plugin['class'] = 'active';
			} else {
				$seva_lite_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$seva_lite_recommended_plugin['button_class'] = 'install-now button';
					$seva_lite_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$seva_lite_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$seva_lite_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$seva_lite_recommended_plugin['button_class'] = 'deactivate-now button';
					$seva_lite_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$seva_lite_recommended_plugin['id'] = $slug;
			$seva_lite_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$seva_lite_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$seva_lite_recommended_plugin['description'] = $info->short_description;
			}

			$seva_lite_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $seva_lite_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="seva-lite-recommeded-actions-container seva-lite-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="seva-lite-recommeded-actions">
