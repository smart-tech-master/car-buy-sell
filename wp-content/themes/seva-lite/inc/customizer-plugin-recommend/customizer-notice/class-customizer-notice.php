<?php
/**
 * Seva Lite Customizer Notification System.
 *
 * @package Seva_Lite
 */
 ...
 */
class Seva_Lite_Customizer_Notice {
	...
	/**
	 * The single instance of Seva_Lite_Customizer_Notice
	 *
	 * @var Seva_Lite_Customizer_Notice $instance The Seva_Lite_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Seva_Lite_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Seva_Lite_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Seva_Lite_Customizer_Notice ) ) {
			self::$instance = new Seva_Lite_Customizer_Notice;
	...
	public function setup_config() {
		...
		$seva_lite_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$seva_lite_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'seva-lite-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'seva-lite-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), SEVA_LITE_THEME_VERSION, true );
		wp_localize_script(
			'seva-lite-customizer-notice', 
            'customizer_notice_data', 
            array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'seva-lite' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the Seva_Lite_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Seva_Lite_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Seva_Lite_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'seva_lite_show_recommended_plugins', $show_recommended_plugins );
		}
