/*jshint -W065 */
var sevaliteRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['seva-lite-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
		...
		// The current value set in Control Class (set in Seva_Lite_Control_Repeater::to_json() function)
	...
	setImageInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.seva-lite-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.seva-lite-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ){
		...
		$targetDiv.find( '.seva-lite-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ){
		...
		$targetDiv.find( '.seva-lite-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ){
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Seva_Lite_Control_Repeater::render_content() ).
		...
		if ( template ) {
			...
			newRow = new sevaliteRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'seva_lite_get_fontawesome_ajax',
					seva_lite_customize_nonce: seva_lite_customize.nonce
				},
				beforeSend: function(){
