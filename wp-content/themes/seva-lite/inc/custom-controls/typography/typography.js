 ...
 */
wp.customize.controlConstructor['seva-lite-typography'] = wp.customize.Control.extend( {
    ready: function(){        
        'use strict';
		...
		renderSubControl = function( fontFamily, sub, startValue ) {
			...
			if ( true === isStandard ) {
				...
				if ( 2 > subList.length ) {
					...
					control.container.find( '.seva-lite-variant-wrapper' ).css( 'display', 'none' );
				} else {
					control.container.find( '.seva-lite-variant-wrapper' ).css( 'display', 'block' );
				}
