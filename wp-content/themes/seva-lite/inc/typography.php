<?php
/**
 * Seva Lite Typography Related Functions
 *
 * @package seva_lite
 */

if( ! function_exists( 'seva_lite_fonts_url' ) ):
/**
 * 
*/ 
function seva_lite_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'seva-lite' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'seva-lite' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'seva-lite' );
...
if( ! function_exists( 'seva_lite_get_google_fonts' ) ) :
/**
 * Get Google Fonts
*/
function seva_lite_get_google_fonts(){
...
if( ! function_exists( 'seva_lite_get_websafe_font' ) ) :
/**
 * Function listing WebSafe Fonts and its attributes
*/
function seva_lite_get_websafe_font(){
    ...
    return apply_filters( 'seva_lite_standard_fonts', $standard_fonts );
}
endif;

if( ! function_exists( 'seva_lite_check_varient' ) ) :
/**
 * Checks for matched varients in google fonts for typography fields
*/
function seva_lite_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = seva_lite_get_google_fonts(); //Google Fonts
    $websafe_fonts = seva_lite_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function seva_lite_get_css_variant( $font_variant ){
    $v_array = array(
		'100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function seva_lite_is_google_font( $font ){
    $return = false;
    $websafe_fonts = seva_lite_get_websafe_font();
    if( $font ){
...
*/
function seva_lite_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = seva_lite_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( seva_lite_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = seva_lite_get_css_variant( seva_lite_check_varient( $font_family, $font_variant ) );
...
if( ! function_exists( 'seva_lite_get_all_fonts' ) ) :
/**
 * Return Web safe font and google font without variants
*/
function seva_lite_get_all_fonts(){
    $google = array();        
    $standard = apply_filters( 'seva_lite_standard_font', array(
		'georgia-serif'       => __( 'Georgia', 'seva-lite' ),
        'palatino-serif'      => __( 'Palatino Linotype, Book Antiqua, Palatino', 'seva-lite' ),
        'times-serif'         => __( 'Times New Roman, Times', 'seva-lite' ),
        'arial-helvetica'     => __( 'Arial, Helvetica', 'seva-lite' ),
        'arial-gadget'        => __( 'Arial Black, Gadget', 'seva-lite' ),
		'comic-cursive'       => __( 'Comic Sans MS, cursive', 'seva-lite' ),
		'impact-charcoal'     => __( 'Impact, Charcoal', 'seva-lite' ),
        'lucida'              => __( 'Lucida Sans Unicode, Lucida Grande', 'seva-lite' ),
        'tahoma-geneva'       => __( 'Tahoma, Geneva', 'seva-lite' ),
		'trebuchet-helvetica' => __( 'Trebuchet MS, Helvetica', 'seva-lite' ),
		'verdana-geneva'      => __( 'Verdana, Geneva', 'seva-lite' ),
        'courier'             => __( 'Courier New, Courier', 'seva-lite' ),
        'lucida-monaco'       => __( 'Lucida Console, Monaco', 'seva-lite' ),
	) );
