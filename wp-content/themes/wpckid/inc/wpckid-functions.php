...
if ( ! function_exists( 'wpckid_is_woo_activated' ) ) {
	/**
	 * Query WooCommerce activation
	 */
	function wpckid_is_woo_activated() {
		return class_exists( 'WooCommerce' ) ? true : false;
	}
}

function wpckid_is_active_sidebar() {
	if ( wpckid_disable_sidebar() ) {
		return false;
	}

	if ( wpckid_is_woo_activated() && is_woocommerce() && ! is_active_sidebar( 'sidebar-shop' ) ) {
		return false;
	}
...
function wpckid_disable_sidebar() {
	if ( ( wpckid_is_woo_activated() && ( is_cart() || is_checkout() || is_account_page() ) ) || apply_filters( 'wpckid_disable_sidebar', false ) ) {
		return true;
	}
 ...
 *
 */
function wpckid_do_shortcode( $tag, array $atts = array(), $content = null ) {
 ...
 * Accounts for the WPCkid Designer and WPCkid Powerpack content background option.
 *
 * @return string the background color
 * @since  1.6.0
 */
function wpckid_get_content_background_color() {
	if ( class_exists( 'WPCkid_Designer' ) ) {
		$content_bg_color = get_theme_mod( 'sd_content_background_color' );
		$content_frame    = get_theme_mod( 'sd_fixed_width' );
	}

	if ( class_exists( 'WPCkid_Powerpack' ) ) {
	...
	if ( class_exists( 'WPCkid_Powerpack' ) || class_exists( 'WPCkid_Designer' ) ) {
...
/**
 * Apply inline style to the WPCkid header.
 *
 * @uses  get_header_image()
 * @since  2.0.0
 */
function wpckid_header_styles() {
	...
	$styles = apply_filters( 'wpckid_header_styles', $styles );
 ...
 * @since  1.0.0
 */
function wpckid_adjust_color_brightness( $hex, $steps, $opacity = 1 ) {
 ...
 * @since  1.3.0
 */
function wpckid_sanitize_choices( $input, $setting ) {
...
add_filter( 'the_content_more_link', 'wpckid_continue_reading_link' );
