...
(
	function () {
		...
		document.addEventListener( 'DOMContentLoaded', function () {
			const container = document.getElementById( 'site-navigation-handheld' );
			if ( !container ) {
				return;
			}
			...
			button.addEventListener( 'click', function () {
				...
				const expanded = container.classList.contains( 'toggled' ) ? 'true' : 'false';
			...
			if ( handheld.length > 0 ) {
				[].forEach.call( handheld[0].querySelectorAll( '.menu-item-has-children > a, .page_item_has_children > a' ), function ( anchor ) {
					...
					btnSpan.appendChild( document.createTextNode( wpckidScreenReaderText.expand ) );
					...
					btn.addEventListener( 'click', function () {
						...
						btnSpan.appendChild( document.createTextNode( expanded ? wpckidScreenReaderText.collapse : wpckidScreenReaderText.expand ) );
					...
					anchor.addEventListener( 'pointerup', function ( event ) {
						if ( anchor.classList.contains( 'blocked' ) || event.pointerType === 'mouse' ) {
							acceptClick = true;
							            ...
							            anchor.className === 'header-cart' && anchor.parentNode.nextElementSibling && anchor.parentNode.nextElementSibling.textContent.trim() !== ''
